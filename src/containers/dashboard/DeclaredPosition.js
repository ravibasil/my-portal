import React from 'react'

export default function DeclaredPosition() {
  return (
    <div className="card" style={{ width: '50%' }}>
      <div className="position-row">
        <div className="position-list">
        </div>
        <div className="interview-box-right">
        </div>
      </div>
      <h4 className="card-title">Declared Positions</h4>
      <div className="position-wrap">
        <div className="position-box bg-danger">
          <div className="card-body  p-4">
            <div className="media">
              <span className="mr-3">
                <img src="assets/images/java.svg" alt="" />
              </span>
              <div className="media-body text-white text-right">
                <p className="mb-1">Java Developer, 2 Years, Ui</p>
                <div className="progress">
                  <div className="progress-bar" role="progressbar" style={{ width: '25%' }} aria-valuenow={25} aria-valuemin={0} aria-valuemax={100} />
                </div>
                <h3 className="text-white">5</h3>
              </div>
            </div>
          </div>
        </div>
        <div className="position-box bg-success">
          <div className="card-body  p-4">
            <div className="media">
              <span className="mr-3">
                <img src="assets/images/re.png" alt="" />
              </span>
              <div className="media-body text-white text-right">
                <p className="mb-1">React Developer, 2 Years, Ui</p>
                <div className="progress">
                  <div className="progress-bar" role="progressbar" style={{ width: '25%' }} aria-valuenow={25} aria-valuemin={0} aria-valuemax={100} />
                </div>
                <h3 className="text-white">5</h3>
              </div>
            </div>
          </div>
        </div>
        <div className="position-box bg-info">
          <div className="card-body  p-4">
            <div className="media">
              <span className="mr-3">
                <img src="assets/images/html.png" alt="" />
              </span>
              <div className="media-body text-white text-right">
                <p className="mb-1">HTML Developer, 2 Years, Ui</p>
                <div className="progress">
                  <div className="progress-bar" role="progressbar" style={{ width: '25%' }} aria-valuenow={25} aria-valuemin={0} aria-valuemax={100} />
                </div>
                <h3 className="text-white">5</h3>
              </div>
            </div>
          </div>
        </div>
        <div className="position-box bg-primary">
          <div className="card-body  p-4">
            <div className="media">
              <span className="mr-3">
                <img src="assets/images/java.svg" alt="" />
              </span>
              <div className="media-body text-white text-right">
                <p className="mb-1">Java Developer, 2 Years, Ui</p>
                <div className="progress">
                  <div className="progress-bar" role="progressbar" style={{ width: '25%' }} aria-valuenow={25} aria-valuemin={0} aria-valuemax={100} />
                </div>
                <h3 className="text-white">5</h3>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}
